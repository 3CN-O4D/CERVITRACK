...
CREATE POLICY "Users update own chat_messages" ON chat_messages FOR UPDATE USING (EXISTS (SELECT 1 FROM chat_conversations cc WHERE cc.id = chat_messages.conversation_id AND cc.user_id = auth.uid()) AND chat_messages.sender_id = auth.uid()) WITH CHECK (EXISTS (SELECT 1 FROM chat_conversations cc WHERE cc.id = chat_messages.conversation_id AND cc.user_id = auth.uid()) AND chat_messages.sender_id = auth.uid());
