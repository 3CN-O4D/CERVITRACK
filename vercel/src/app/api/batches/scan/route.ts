...
export async function POST(req: NextRequest) {
  ...
  const batch = getBatchByCode(batch_code);
  ...
  const result = addSampleToBatch(batch.id, {
    kit_barcode,
    kit_id: kit.id,
    patient_id: kit.patientId,
    patient_name: kit.patientName,
  });

  if (result.error) return NextResponse.json({ message: result.error }, { status: result.status });
  return NextResponse.json({ batch: result.batch, kit, patient_name: kit.patientName });
}
